...
vector<bool> all_compatible_pairs(int length_a, int state_a, int length_b, vector<int> &states_b, grid_t &test_grid)
{
	vector<bool> res(states_b.size());
...
vector<vector<vector<vector<bool>>>> find_all_nonadj_lists(vector<vector<int>> &valid_states)
{
	...
	vector<vector<vector<vector<bool>>>> res(n);
	for (int i = 0; i < n; ++i)
	{
...
void gen_cnf(vector<int> num_valid_states, vector<vector<vector<vector<bool>>>> &validity_masks)
{
	vector<string> bitvec(n); // index of bitvector
	for (int i = 0; i < n; ++i)
		bitvec[i] = i;
	...
	{
		cout << "0 <= " << i << '\n';
		cout << i << " < " << num_valid_states[i] << '\n';
	}
	for (int i = 0; i < n; ++i)
		for (int state_i = 0; state_i < num_valid_states[i]; ++state_i)
			for (int j = i+1; j < n; ++j)
				for (int state_j = 0; state_j < num_valid_states[j]; ++state_j)
					if (!validity_masks[i][state_i][j][state_j])
						cout << "(" << i << " != " << state_i << ") or (" << j << " != " << state_j << ")\n";
}
...
void count_occurrences(grid_t &misses)
{
	...
	vector<vector<vector<vector<bool>>>> validity_masks = find_all_nonadj_lists(valid_states);

	gen_cnf(num_valid_states,validity_masks);
